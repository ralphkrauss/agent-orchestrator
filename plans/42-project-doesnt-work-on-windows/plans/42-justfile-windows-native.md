...
present. It does **not** show the `[script]` attribute (which was
introduced in `just 1.33.0`). T0 verifies this version floor before
...
| 4 | Just version floor | Document `just >= 1.33` as the conservative floor. The plan uses `[script("node")]` and `set windows-shell` (since 0.10); the parse-time `repo_url` variable from the prior draft is removed (decision #7), so `os()`, `replace()`, and conditional `if/else` are no longer load-bearing. The empty `[script]` attribute (no command) was added in 1.33; `[script(COMMAND)]` (the form this plan uses) was added in 1.32 (verified via the just changelog). `>= 1.33` is therefore one minor version more conservative than strictly necessary; the plan keeps it that way to (a) avoid pinning to a feature-flag-shape that might have shifted between 1.32 and 1.33 and (b) match the version most Windows package managers had cached when the plan was written. The local `just` is 1.21.0 and must be upgraded on the implementation host before T0 can complete; T0 documents the upgrade. | Reviewer correction: `[unix]`/`[windows]` are gated at 1.8.0, not 1.5. `[script(COMMAND)]` is gated at 1.32. `>= 1.33` is the conservative choice. The cost (`just` minor-version upgrade) is small relative to (α)'s new `pwsh` requirement. | (1.32 floor) — saves nothing material and risks obscure version-skew bugs in the boundary minor. (1.5 floor) — drops `[script]` and falls back to the broken PowerShell `[windows]` design. (1.21 floor) — same. |
| 5 | `set positional-arguments` | **Drop.** Once parameter-taking recipes use `[script("node")]`, no recipe body invokes a shell with positional args. Class 1 recipes have no parameters. No-param Class 3 recipes have no parameters. The directive is dead code under decision #3; removing it eliminates a future maintenance trap (a contributor adding a new shell-based parameter-taking recipe under the impression that `set positional-arguments` will protect it on Windows). | The argv-safety property previously provided by `set positional-arguments` is now provided by `[script]` skipping shell tokenization entirely. The directive is no longer load-bearing. | Keep it as defense-in-depth — would imply a future contributor might re-introduce shell-based parameter-taking recipes, which the top-of-file invariant comment (decision #20) explicitly forbids. Keeping a dead setting muddles the contract. |
| 6 | `[script("node")]` argv semantics | The recipe body inside `[script("node")]` runs as a Node script: `just` writes the body to a tempfile and invokes `node <tempfile> <recipe-arg-1> <recipe-arg-2> ...`. Inside the body, `process.argv[0]` is the Node executable, `process.argv[1]` is the tempfile path, and `process.argv.slice(2)` is the recipe arguments — `*args` expands to multiple entries; named parameters (`branch`, `days`) each occupy one entry; the recipe NAME is **not** in `process.argv`. T0 verifies these semantics with a probe recipe before any production rewrite lands. If T0 disproves any part of this, halt and re-plan. | Documented `[script]` behavior; matches Node shebang semantics that the existing `scripts/*.mjs` files already rely on. | Assuming process.argv[2] is the recipe name (incorrect — that pattern is specific to `set positional-arguments` + shell, not `[script]`). |
...
| 13 | Test execution scope | Add a new Quality Gate command `node --test scripts/just/__tests__` (directory form so Node walks the directory; avoids reliance on shell glob semantics differing between PowerShell and `sh`). Document the gate explicitly in the Quality Gates section. Do **not** modify `package.json`'s `test` script. The `pnpm test` scope (compiled `dist/__tests__/`) is unchanged. | Reviewer answer to B3 + Q7. Adds visible test coverage without broadening a long-stable script. | Modifying `package.json`'s `test` script; relying silently on `pnpm test` to cover `scripts/just/__tests__/`. |
...
| 19 | README updates | Replace the existing "The repository's `justfile` recipes also rely on POSIX shell idioms..." paragraph (README.md L76–80) with: "On Windows, `just` recipes run under PowerShell Desktop and through `node` directly for parameter-taking recipes (via `just`'s `[script]` attribute, which requires `just >= 1.33`); no Git Bash or `sh.exe` is required for the dev workflow. The Git Bash requirement for `agent-orchestrator claude` itself (described above) is unchanged because Claude Code uses Bash for its `Bash` tool." Keep the existing Claude-launcher Git Bash paragraph (L69–74) **verbatim**. | Decision #4 made the `just >= 1.33` floor user-visible; the README must call it out so a fresh-checkout developer knows what to install. The two Git Bash concerns are kept distinct. | Hiding the version floor; collapsing the two paragraphs. |
| 20 | New helpers location | New helpers under `scripts/just/`. Shared local-home module at `scripts/local-home-lib.mjs` (top-level because two existing scripts also import it). Top-of-file comment in the `justfile` documents the cross-shell invariant: "Recipe bodies must be (a) a single external-program invocation that works in any shell, or (b) `[script("node")]` recipes that import a helper under `scripts/just/`. Never use POSIX-only shell idioms (`[ ... ]`, `printf`, `find`, `rm`, `\|`, `&&`) in a recipe body. `just >= 1.33` required (`[script]` attribute)." | Reviewer answer to Q6 + Risk #11 mitigation. | All flat. |
...
- `just >= 1.33` honors the `[script("node")]` attribute by writing
  the recipe body to a tempfile and running
  `node <tempfile> <recipe-arg-1> <recipe-arg-2> ...`, with
  `process.argv[0]` being the Node executable, `process.argv[1]`
  being the tempfile, and `process.argv.slice(2)` being the recipe
  args. The recipe name is not in `process.argv`. `*args`, named
  parameters, and defaulted parameters all flow through the same
  positional-args mechanism. (Documented `[script]` behavior; T0
  verifies all three parameter shapes.)
...
- Node 22+'s built-in test runner (`node --test`) accepts a directory
  path and walks it for `*.test.{mjs,cjs,js}`. (Documented.)
- The local `just` is 1.21.0, below the 1.33 floor. The implementer
  upgrades to `just >= 1.33` before T0; the upgrade is a single
...
- **`just` recipe execution does not require Git Bash.** A fresh
  ...
  `just >= 1.33` on `PATH` (and no Git Bash) can run every recipe.
...
- **Reintroducing `set positional-arguments`** (decision #5 dropped
  it; reintroduction would be a sign of regressing toward the
  prior-draft design).
...
- Q1 → decisions #3 + #4 (now resolves to Option γ, `[script("node")]`,
  with `just >= 1.33` floor).
- Q2 → decision #2 (PowerShell Desktop pin retained for Class 1 and
  no-param Class 3).
- Q3 → decision #5 (drop `set positional-arguments` — now safe and
  appropriate because `[script]` skips the shell entirely).
- Q4 → decisions #14 + #16 (drop the parse-time backtick; introduce
  shared local-home module).
- Q5 → decision #4 (just version floor `>= 1.33`).
...
- Second-pass `[unix]`/`[windows]` 1.8 floor correction → moot under
  ...
  documents the actual 1.33 floor.
...
- Third-pass minor cleanups → decision #4 footnote (`[script(COMMAND)]`
  is actually 1.32; conservative 1.33 retained); T0 expanded with
...
| 2 | A user invokes `just` with a `just` version below 1.33 (the floor). | The `justfile` parser will fail with an error referencing the unknown attribute. `just` 1.33+ is documented as the floor in the top-of-file invariant comment (decision #20) and in the README rewrite (decision #19). T7 manual smoke verifies behavior on a 1.33 host. | Documentation; T7. |
...
| 5 | A Class 1 recipe still passes user input to a shell because of an oversight (e.g. a future contributor adds `*args` to `orchestrator-help`). | Decision #5 dropped `set positional-arguments`, so any future Class 1 recipe with `*args` would interpolate `{{args}}` directly into the PowerShell-or-`sh` command — visibly broken on the first dry-run test. T6 sweeps every parameter-taking recipe; a regressing change would fail T6 immediately. | T6 + top-of-file invariant comment (decision #20). |
...
| 11 | A future contributor reintroduces a POSIX-only recipe and re-introduces a `sh.exe` requirement. | Top-of-file invariant comment (decision #20) explicitly forbids POSIX-only shell idioms in recipe bodies and documents the `>= 1.33` floor. Not enforced in CI. | Top-of-file comment. |
| 12 | A PowerShell-Desktop-only Windows host attempts to use `just` 1.33+ — does the `[script]` mechanism work without `pwsh`? | `[script("node")]` invokes `node` directly; PowerShell Desktop is unrelated to the script-recipe path. PowerShell is only used for Class 1 and no-param Class 3 bodies (which contain no parameter interpolation). T7 verifies on a PowerShell-Desktop-only host. | T7. |
...
| 16 | A fresh Windows developer does not have `just >= 1.33` on `PATH` (e.g. they installed an older `just` from chocolatey). | The README rewrite (decision #19) documents the 1.33 floor. The `justfile` parse fails with a clear error if the floor is not met. | Documentation; user-visible parse error. |
...
|---|---|---|---|
| T0 | Verify `[script("node")]` argv semantics + `pathToFileURL` resolution + cwd assumption + just version | none | (a) Upgrade local `just` to `>= 1.33` (the implementation host's `just` is 1.21.0); record the upgraded version in the implementation log via `just --version`. (b) **`*args` probe.** Land a temporary probe recipe `_probe-args *args:` with the `[script("node")]` attribute and body `console.log(JSON.stringify(process.argv));`. Run `just _probe-args -- 'a;b' 'c d' '$x' '"q"'` on both Linux and Windows (PowerShell Desktop) and capture `process.argv`. Assert `process.argv.slice(2)` equals `['--', 'a;b', 'c d', '$x', '"q"']` exactly (recipe name not in argv; tempfile path at `process.argv[1]`). (c) **Named-parameter probe.** Land `_probe-named branch:` with `[script("node")]` and the same body. Run `just _probe-named main` on both platforms and assert `process.argv.slice(2)` equals `['main']`. Run `just _probe-named 'feature/foo;bar'` and assert `process.argv.slice(2)` equals `['feature/foo;bar']` (the `;` is data, not a separator). (d) **Defaulted-parameter probe.** Land `_probe-default days="30":` with `[script("node")]` and the same body. Run `just _probe-default` (no arg) on both platforms and assert `process.argv.slice(2)` equals `['30']`. Run `just _probe-default 7` and assert `['7']`. Run `just _probe-default '13;rm'` and assert `['13;rm']`. (e) **`pathToFileURL` resolution probe.** Land `_probe-pathurl:` with `[script("node")]` and body `const { pathToFileURL } = require('node:url'); const { resolve } = require('node:path'); console.log(pathToFileURL(resolve('justfile')).href);`. Run on both platforms; assert the printed URL is a valid `file://` URL pointing at the actual `justfile` location. On Windows this proves backslash normalization and any-character percent-encoding work end-to-end. On Linux assert the path resolves identically to `pwd`/`justfile`. (f) **Working-directory probe (verifies the cwd assumption that decision #7 relies on).** Land `_probe-cwd:` with `[script("node")]` and body `console.log(process.cwd());`. Run from `justfile_directory()` and from a subdirectory (`cd subdir && just _probe-cwd`). Assert `process.cwd()` equals `justfile_directory()` in both cases (i.e. just changes cwd to `justfile_directory()` before running `[script]` recipes, matching documented just behavior). If `[script]` does not change cwd, halt and switch decision #7 to use `justfile_directory()` injected via env var (`export JUSTFILE_DIR := justfile_directory()` and `path.resolve(process.env.JUSTFILE_DIR, 'scripts/just/cli.mjs')`). (g) **Signal-table sanity.** In one of the probes (or a small Node one-liner under `node -e`), verify `require('node:os').constants.signals['SIGTERM']` returns a number on Linux and is `undefined` (or absent) on Windows. (h) Remove all probe recipes after verification. Evidence (probe outputs, just version) captured in the implementation log. **If any assertion fails, halt and re-plan; do not silently fall back to any shell-based approach.** |
...
- `set positional-arguments` is **dropped**; `set windows-shell` is
...
- README L76–80 reflects the new state and surfaces the `just >= 1.33`
  floor; existing L69–74 Claude-launcher Git Bash paragraph preserved
  verbatim.
- `pnpm build`, `pnpm test`, and the new `node --test scripts/just/__tests__`
...
- [ ] T0 evidence captured: `just --version >= 1.33`; `[script("node")]`
      `*args` probe; named-parameter probe; defaulted-parameter probe;
...
- [ ] `node --test scripts/just/__tests__` succeeds (new explicit
