...
function orchestrationPrompt(input: OpenCodeHarnessConfigInput): string {
  return [
    ...
    '',
    'Long-running run supervision:',
    '- Use bounded wait_for_run calls rather than one very long wait. Make the first check-in around 30 seconds after start_run so startup, auth, model, quota, and protocol failures surface quickly.',
