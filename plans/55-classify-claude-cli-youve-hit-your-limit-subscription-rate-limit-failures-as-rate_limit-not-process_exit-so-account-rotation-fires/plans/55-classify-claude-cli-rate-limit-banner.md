...
| 3 | Regex shape (RQ3 resolved — keep narrow; F1 tightened 2026-05-11) | Match an explicit Claude-CLI banner phrase with a **mandatory** tail-anchor: `/\byou(?:'|ʼ|’|`)ve\s+(?:hit|reached)\s+your\s+(?:usage\s+|rate\s+|monthly\s+)?limit\b(?=\s*(?:[·•]\s*resets\b|[.!?…]?\s*$))/i`. The lookahead requires the match to be followed by **either** (i) the banner's distinctive resets clause separator `·` (U+00B7 middle dot — the character observed in the issue evidence — or `•` U+2022 bullet) plus the literal word `resets`, **or** (ii) end-of-string with at most one terminal punctuation mark (`.`, `!`, `?`, or `…`). Run the match against the trimmed `resultEvent.summary` only. Do not preemptively widen to marketing strings ("subscription paused", "usage cap reached") without captured fixtures. | Anchored on "you've ... your limit" so user-prompt text that merely mentions the word "limit" cannot misfire. The new tail-anchor explicitly **rejects** continuations like `"You've hit your limit of 5 retries"`, `"You've hit your limit for the day"`, `"You've hit your limit (this is just a draft)"` — i.e. any text where the word `limit` is followed by content other than the banner's resets clause or terminal punctuation. This closes the medium-severity finding F1 from the 2026-05-11 plan review (recorded under `reviews/review-2026-05-11-plan.md`), which observed that the prior `limit\b` anchor would mis-classify error-shaped result events containing such phrasing once the structured-failure gate fires. Curly-quote variant and the `hit|reached` alternation cover the wordings observed in the issue evidence and historical CLI text. The middle-dot `·` is the separator captured in the issue's evidence string; the bullet `•` is included as the close lexical neighbour. Banner widening can ride on the same hook when a future operator submits a captured fixture. | A. Strict literal `^You've hit your limit\b` (rejected: misses curly-quote variants observed in real CLI output and the equivalent "reached your limit" wording). B. Pattern-match any "resets HH:MM" string (rejected: too lax; assistant-authored markdown commonly contains time strings). C. Locale-aware regex (rejected: Claude CLI ships in English; no captured non-English samples). D. The previously-planned `limit\b`-only anchor without the tail-lookahead (rejected on F1: passes for `"You've hit your limit of 5 retries"` once the structured-failure gate fires, mis-classifying an arbitrary retry-counter message as `rate_limit` and cooling down/rotating an account that did not actually hit its subscription cap). |
...
| 4 | A real successful `result` event contains banner-shaped prose (e.g. assistant explains why a tool said "you've hit your limit of 5 retries"). | **Two layers of defence.** (1) Structured-failure gate (Decision 2) refuses synthesis when `is_error !== true && subtype !== 'error' && stop_reason !== 'rate_limit_error'`. (2) Tail-anchored regex (Decision 3) refuses to match `"...your limit of 5 retries"` / `"...your limit for the day"` even when the gate fires, because the lookahead requires either the banner's `· resets` clause or end-of-string with at most terminal punctuation. So a *failed* result event carrying the same phrasing is also safe (this is the medium-severity finding F1 in the 2026-05-11 plan review). | Decision 2 + Decision 3 + T3 (F1 negative cases) + T4 case (e). |
...
| 8 | A test fixture writes a banner-shaped string into a result event for an unrelated test, accidentally triggering rate_limit classification. | Decision 3's regex anchors on "you've (hit/reached) your limit" **and** requires the trailing `· resets ...` clause or end-of-string (tail-anchor lookahead); Decision 2's structural gate additionally requires `is_error`/`subtype: 'error'`/`stop_reason: 'rate_limit_error'`. A pre-implementation grep confirms no existing fixture matches all three gates simultaneously. The tail-anchor also rules out the most likely accidental overlap (`"limit of N retries"` shapes). | Pre-implementation grep + Decision 2 + Decision 3 + T3 / T4 negative cases. |
